...
class webSerial {
    ...
    setupSelect(parentId) {
        if(chrome.serial){
            ...
            document.getElementById('connectSerial').onclick = () => {
                ...
                this.connectSelected(true, document.getElementById('serialports').option, document.getElementById('serialports').value); 
            }
        }
        else if(navigator.serial){
            var frag = document.createDocumentFragment();
    ...
    connectSelected(connect=true, deviceName='',devicePath='') { //Set connect to false to disconnect  
        if ((connect == true) && (devicePath != '')) {
            console.log("Connecting", devicePath);
            if(deviceName.indexOf('STM') > -1){ chrome.serial.connect(devicePath, {bitrate: 921600}, this.onConnectComplete);}
            else{chrome.serial.connect(devicePath, {bitrate: 115200}, this.onConnectComplete);}
        } else {
    ...
    async onPortSelected(port) {
        ...
        catch {await port.open({ baudRate: 115200 });} //API inconsistency between linux and windows
        this.finalCallback();
        this.subscribe(port);
    }
