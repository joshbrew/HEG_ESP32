 ...
 * - Fix usb and bluetooth concurrency when bluetooth slower than usb (as in always)
 * - Make pIR mode better (as in turn off both LEDs and use a different core program altogether).
 */
...
bool pIR_MODE = false; // SET TO TRUE OR WRITE 'p' TO DO PASSIVE INFRARED ONLY (NO RED LIGHT FOR BLOOD-OXYGEN DETECTION)
...
const unsigned long samplesPerRatio = 10; // Number of samples per LED to accumulate before making a measurement. Time = 2*samples*sampleRate
            ...
            if(signalDetermined == false){
              ...
              if(ticks0 > 500) { // Wait for 500 samples of good signal before getting baseline
                ...
                if((ticks1 < 500) && (ticks2 < 500)) { // Accumulate samples for baseline
                  ...
                  baseline = redavg * 100 / iravg; // Set baseline ratio
