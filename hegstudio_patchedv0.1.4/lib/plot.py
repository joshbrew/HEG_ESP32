...
#
# Created:     2003/11/03
# RCS-ID:      $Id: plot.py 65713 2010-10-01 17:56:49Z RD $
# Copyright:   (c) 2002
...
class PolyPoints:
    ...
    def __init__(self, points, attr):
        self._points = _Numeric.array(points)
...
class PlotCanvas(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        self.canvas.SetCursor(wx.Cursor(wx.CURSOR_ARROW))#CROSS_CURSOR)
        ...
        # Memory for mouse button state.
        # Workaround for bug in which ButtonUp events aren't received if the
        # mouse button is lifted while the mouse is not over the canvas.
        self._leftWasDown = False
        ...
        self._gridColour = self.GetForegroundColour()#wx.NamedColour('black')
    ...
    def SetEnableDrag(self, value):
        ...
        if value:
            ...
            self.SetCursor(wx.StockCursor(wx.CURSOR_ARROW))#CROSS_CURSOR)
    ...
    def Draw(self, graphics, xAxis = None, yAxis = None, dc = None):
        ...
        if type(yAxis) not in [type(None),tuple]:
            raise TypeError, "yAxis should be None or (minY,maxY)"+str(type(yAxis))
    ...
    def _Draw(self, graphics, xAxis = None, yAxis = None, dc = None):
        ...
        if wx.version() < '4': dc.BeginDrawing()
        # dc.Clear()
    ...
    def OnMotion(self, event):
        if self._zoomEnabled and (event.LeftIsDown() or self._leftWasDown):
            if self._hasDragged:
        ...
        elif self._dragEnabled and (event.LeftIsDown() or self._leftWasDown):
        ...
        if self._leftWasDown and not event.LeftIsDown():
            self.OnMouseLeftUp(event)
        if not self._leftWasDown and event.LeftIsDown():
            self.OnMouseLeftDown(event)
            
    def OnMouseLeftDown(self,event):
        self._leftWasDown = True
    ...
    def _ticks(self, lower, upper, numticks=None):
        if isinstance(numticks, (float, int)):
            ...
            if numticks.startswith('auto') and numticks[4:].isdigit():
                count = float(numticks[4:])
            else:
                count = 5.
            ideal = (upper-lower)/count
